...
import {
  adjustDecisionForNutrition,
  getNutritionDiagnosis,
} from "@/lib/nutritionDiagnosis";
...
`;
  ...
  const readinessAdjustedDecision = adjustDecisionForReadiness(
    freshnessAdjustedDecision,
    readinessScore
  );
  const agentDecision = adjustDecisionForNutrition(
    readinessAdjustedDecision,
    nutritionDiagnosis
  );
  ...
  const agentMemory = getAgentMemory({
    ...
    avgSteps,
    avgCalories,
    currentDecision: agentDecision.action,
    trainingSignal,
    loggingQuality,
  });
