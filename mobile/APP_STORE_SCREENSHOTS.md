# FitCheck Coach App Store Screenshots

Use fictional data only. Do not include personal logs, personal goals, real private notes, or screenshots from the personal web app.

## Required Story

1. Today screen: blank-friendly logging for weight, calories, protein, steps, and workout type.
2. Progress screen: goal-aware weight trend, 7-log averages, 14-log nutrition diagnosis, and chart previews.
3. Goals screen: cutting, maintaining, and bulking setup with targets.
4. Training screen: workout logging with exercises, sets, reps, load, bodyweight work, and form-focus notes.
5. Settings screen: local backup, restore, sign out, reset, and privacy context.

## Screenshot Copy

- Log What You Know
- Stay Goal-Aware
- Track Trends, Not Noise
- Review Training Progress
- Local-First Fitness Logs

## QA Before Capturing

- Use demo or fictional mobile data only.
- Confirm blank fields are not displayed as zero.
- Confirm no OpenAI/API feature appears in the mobile app.
- Confirm app icon and splash screen match the release assets.
- Capture with consistent device size, light mode, and clean status bar.
